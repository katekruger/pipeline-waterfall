...
- `waterfall__movement_detail`'s undocumented columns (`entity_id`, `source_relation`, `period_grain`, `period_start`, `period_end`, `stage`, `currency_code`) now have schema.yml descriptions, completing column coverage for every model a user is expected to query directly (`int_waterfall__daily_state`, `waterfall__movement_detail`, `waterfall__bridge_monthly`, `waterfall__bridge_quarterly`).
- Quarterly grain (half of `waterfall__period_grain`'s default) had never once run against real data in CI -- the integration fixtures spanned one real month, and `integration_tests/dbt_project.yml` separately overrode the grain list down to month-only. Both are fixed: the seed now spans two real quarters (through 2026-07-01) and the override is removed. A new fixture and integration test prove quarter-boundary classification is genuine, not incidental. See ADR 0009.
...
- `dbt compile` and `dbt docs generate` no longer fail against a cold/empty warehouse -- the compile-time `run_query()` calls used to resolve period boundaries and `waterfall__cohort_mode: current_state`'s "latest known snapshot" date are now guarded against a relation that hasn't been built yet, not just against pure static parsing. This package's own unit tests also no longer break the build under `current_state`. See ADR 0007.
- `waterfall__cohort_mode: current_state` broke `bridge_reconciles`/`mart_bridge_reconciles` when run on top of an already-populated database (e.g. right after a default `cohort_at_open` build, without `--full-refresh`) -- `waterfall__movement_detail`'s append-only incremental filter left every previously-loaded period's `bridge_amount` stale relative to what `current_state` means on the current run. Fixed by skipping the incremental filter under `current_state`, relying on the existing `unique_key` to merge-update each period in place. See ADR 0010 (and the correction appended to ADR 0007, whose earlier local testing had incidentally masked this exact bug by always starting from a fresh database).
